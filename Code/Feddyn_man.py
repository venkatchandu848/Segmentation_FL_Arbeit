...
from Fedavg import average_weights, prepare_dataloaders, pad_batch
from Fedavg_man import CustomDeepLabV3, validate, CombinedLoss
...
def plot_iou(training_ious, validation_ious, filename="iou_vs_rounds (FedDyn MAN).png"):
    ...
    plt.title('Training and Validation mIoU vs Communication Rounds for FedDyn MAN model')
...
# Training loop
def train_feddyn_man(model, train_loader, criterion, optimizer, metrics, device, epochs, mu, min_batch_size=4, lambda_reg=0.01):
    model.train()
...
def main_feddyn_man():
    ...
    root_dir = '/data/raml/group1/chandu/mmsegmentation/data/cityscapes' # Root directory for dataset
    num_classes = 20
    batch_size = 4
    num_workers = 4
    local_epochs = 10
    ...
    with open("feddyn_man.txt", "w") as f:
        for round in range(communication_rounds):
            ...
            for client in range(num_clients):
                ...
                client_weights, client_loss, client_iou = train_feddyn_man(client_model, client_dataloaders[client], criterion, client_optimizer, metrics, device, local_epochs, mu)
    ...
    plot_iou(training_ious, validation_ious, filename="iou_vs_rounds (FedDyn MAN).png")
    
    model_save_path = "feddyn_man_model.pth"
